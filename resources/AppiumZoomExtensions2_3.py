...
class AppiumZoomExtensions2_3:
    ...
    @keyword('Universal Zoom On Area')
    def universal_zoom(self, locator=None, scale=2.0, duration_ms=100, pause_s=0.5, attempts=3):
        """Realiza zoom na área de um elemento ou tela inteira com controle preciso
        
        Args:
            locator: (Opcional) Localizador do elemento. Se None, usa tela inteira
            scale: Fator de zoom (deve ser > 1.0)
            duration_ms: Duração do gesto em ms (None = automático)
            pause_s: Pausa após pressionar em segundos (None = 0.3s)
            attempts: Número de tentativas se falhar
        """
        if scale <= 1.0:
    ...
    def _get_element_coordinates(self, locator):
        """Obtém as coordenadas absolutas do elemento"""
    ...
    def _calculate_duration(self, scale):
        ...
        return min(int(base_duration * (scale - 1)), 1000)  # Máximo 1 segundo
    
    def _perform_zoom_gesture(self, center_x, center_y, initial_offset, final_offset, duration_ms, pause_s):
        """Executa o gesto de zoom nas coordenadas especificadas com pausa controlada"""
        ...
        finger2.create_pause(pause_s)  # Pausa configurável aqui

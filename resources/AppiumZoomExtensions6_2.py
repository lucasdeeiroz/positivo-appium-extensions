...
class AppiumZoomExtensions6_2:
    ...
    @keyword("Perform Zoom_3")
    def perform_zoom_in_gesture(self, locator, scale=1.5, duration=50, direction="vertical"):
        """
        ...
        try:
            ...
            # Deslocamento perpendicular para evitar que os dedos estejam no mesmo eixo
            offset = 30  # pixels de separação lateral (ou vertical, dependendo da direção)

            if direction.lower() == "vertical":
                # Deslocamento nos eixos X (lateral)
                finger1_start_x, finger1_start_y = center_x - offset, center_y - 10
                finger2_start_x, finger2_start_y = center_x + offset, center_y + 10
                finger1_end_x, finger1_end_y = center_x - offset, center_y - (center_y * (scale - 1))
                finger2_end_x, finger2_end_y = center_x + offset, center_y + (center_y * (scale - 1))

            elif direction.lower() == "horizontal":
                # Deslocamento nos eixos Y (vertical)
                finger1_start_x, finger1_start_y = center_x - 10, center_y - offset
                finger2_start_x, finger2_start_y = center_x + 10, center_y + offset
                finger1_end_x, finger1_end_y = center_x - (center_x * (scale - 1)), center_y - offset
                finger2_end_x, finger2_end_y = center_x + (center_x * (scale - 1)), center_y + offset

            # Garantir que as posições estão dentro da tela
            def adjust(x, y):
            ...
            # Executa gesto com ActionChains

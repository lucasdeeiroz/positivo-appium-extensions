...
# OBS: O nome da classe deve ser igual ao nome do arquivo (.py), tudo minúsculo e sem underline extra.
class scroll_up:
    # Define o escopo da biblioteca como GLOBAL, tornando-a compartilhada entre todos os testes
    ROBOT_LIBRARY_SCOPE = 'GLOBAL'

    def __init__(self):
        # Instancia a biblioteca BuiltIn para acessar funcionalidades do Robot Framework
    ...
    # Registra o método como uma keyword chamada "Scroll Up Custom" no Robot Framework
    @keyword("Scroll Up Custom")
    def scroll_up_Custom(self, start_ratio=0.2, end_ratio=0.8, duration=500):
        """
        Realiza um gesto de scroll para cima (deslizando de cima para baixo na tela)
        - start_ratio: Posição inicial do swipe como proporção da altura da tela (padrão: 0.2, ou 20%)
        - end_ratio: Posição final do swipe como proporção da altura da tela (padrão: 0.8, ou 80%)
        ...
        self._builtin.log(
            f"Scroll Up de ({x}, {y_start}) para ({x}, {y_end})", 
            "INFO"
        )
